...
int AtualizaGrid(float **grid, float **newgrid, int lines, int j){
    int neighbors;
    int cont_vivos = 0;
...
void start_game(float **grid, float **newgrid) {
    ...
    for(i = 1; i <= EXECUTIONS; i++){
        ...
        #pragma omp parallel for private(k) //precisa por k como privado para cada thread ter sua variavel privada e não tem problema de acessar lugar indevido
            for (j = 0; j < N; j++)         // da mesma forma, j já é privado por padrão
            {
                int cont_vivos = 0;
                for (k = 0; k < N; k++)
                {
                    cont_vivos += AtualizaGrid(grid, newgrid, j, k);   //recebendo a quantidade de vivos de cada local no tabuleiro e somando
                }

                // seção critica pedida para atualizar os vivos (variavel global)
                #pragma omp critical
                {
                    vivos += cont_vivos;   //adiciona a variavel global vivos dentro da seção critica para receber os vivos daquela linha e somar até obter o tabuleiro todo da geração
                }
            }
